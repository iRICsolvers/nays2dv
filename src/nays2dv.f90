...
!
! ï\ñ í£óÕÇÃåvéZ (j_surf==1ÇÃéûÇÃÇ›)
!
        real(8)::smg_g
!
! ----- Surface Pressure ------
...
! ï«èåè(â∫ó¨ÅCâEï«)
    ...
    subroutine pbound
! j_bound=2 .and. j_periodic==1 é¸ä˙ã´äEÇÃéûÇÃÇ›
        do j = 0, ny + 1
            ypn(1, j) = ypn(nx - 1, j)
            ypn(nx, j) = ypn(2, j)
        end do
...
! uÇÃèÍçá
        if (itp == 1) then
            ...
            if (j_bound == 1) then
! ï¬çΩêÖàÊ
                i1 = 2; i2 = nx - 2  !i=1Ç∆i=nx-1ÇÕÉ[ÉçílÇã´äEèåèÇ≈ó^Ç¶ÇÈ
            else if (j_bound == 2) then
!äJï˙êÖàÊ
                i1 = 1; i2 = nx - 1 !i=0Ç∆i=nxÇ…ã´äEèåèÇó^Ç¶ÇÈ(ä‹Çﬁé¸ä˙ã´äE)
            else if (j_bound == 3) then
!è„ó¨äJï˙,â∫ó¨ï«
                i1 = 1; i2 = nx - 2 !i=0Ç…ÇÕã´äEèåèÅCi=nx-1Ç…É[ÉçÇó^Ç¶ÇÈ
            else
!è„ó¨ï«,â∫ó¨äJï˙
                i1 = 2; i2 = nx - 1 !i=1Ç…É[ÉçÇi=nxÇ…ã´äEèåèÇó^Ç¶ÇÈ
            end if
            do i = i1, i2
                hs_up = (hs(i) + hs(i + 1))*.5
                do j = j1, j2
...
!è„â∫ó¨ÇÃuÇÃèåè
        if (j_bound == 2) then !è„â∫ó¨ÉtÉäÅ[ÇÃèÍçá
            if (j_periodic == 1) then !é¸ä˙ã´äEèåè
                do j = 0, ny + 1
                    u(0, j) = u(nx - 2, j)
                    u(nx, j) = u(2, j)
                end do
            else  !è„â∫ó¨Ç™ÉtÉäÅ[ÇÃèÛë‘
                do j = 0, ny + 1
        ...
        else if (j_bound == 1) then !è„â∫ó¨ï¬çΩ
            do j = 0, ny + 1
                u(1, j) = 0.
                u(nx - 1, j) = 0.
            end do
        else if (j_bound == 3) then !è„ó¨äJï˙ÅEâ∫ó¨ÇÕï«
            do j = 0, ny + 1
                u(0, j) = u(1, j)
                u(nx - 1, j) = 0.
            end do
        else if (j_bound == 4) then !â∫ó¨äJï˙ÅEè„ó¨ï«
            do j = 0, ny + 1
                u(1, j) = 0.
                u(nx, j) = u(nx - 1, j)
            end do
...
!       v(nx,j)=0.
        ...
        else if (j_bound == 3) then!è„ó¨äJï˙â∫ó¨ï«
            do j = 0, ny
!       v(1,j)=0.
!       v(nx,j)=0.
                v(1, j) = v(2, j)
        ...
        else if (j_bound == 4) then!â∫ó¨äJï˙è„ó¨ï«
            do j = 0, ny
!       v(1,j)=0.
!       v(nx,j)=0.
                v(1, j) = v(2, j)
...
!-----------------------------------------------
    subroutine bound_c(c, jc_in_l, jc_in_r, jc_in_b, jc_in_u &
        ...
        if ((j_bound == 2 .and. j_periodic == 0) .or. (j_bound == 3)) then !è„ó¨äJï˙
            do j = 1, ny
                if (yu(1, j) > 0.) then
                    c(1, j) = con0
                else
        ...
        if ((j_bound == 2 .and. j_periodic == 0) .or. (j_bound == 4)) then !â∫ó¨äJï˙
            do j = 1, ny
                if (yu(nx - 1, j) < 0.) then
                    c(nx, j) = con0
                else
    ...
    subroutine output(fid, u, v, p, t, c, h, eta_up, qc, xg, yg_bed, obst)
        ...
        do i = 1, nx - 1
            ...
            do j = 0, ny
                ...
                if ((obst(i, j) == 0) .and. (i .gt. 1) .and. (j .gt. 0)) then
                    num = num + 1
                    sum_p = sum_p + p(i, j); sum_t = sum_t + t(i, j); sum_c = sum_c + c(i, j)
                else if ((obst(i + 1, j) == 0) .and. (i .lt. nx - 1) .and. (j .gt. 0)) then
                    num = num + 1
                    sum_p = sum_p + p(i + 1, j); sum_t = sum_t + t(i + 1, j); sum_c = sum_c + c(i + 1, j)
                else if ((obst(i, j + 1) == 0) .and. (i .gt. 1) .and. (j .lt. ny)) then
                    num = num + 1
                    sum_p = sum_p + p(i, j + 1); sum_t = sum_t + t(i, j + 1); sum_c = sum_c + c(i, j + 1)
                else if ((obst(i + 1, j + 1) == 0) .and. (i .lt. nx - 1) .and. (j .lt. ny)) then
                    num = num + 1
...
!--------------------------------------------
    subroutine v1cal(u, v)
        ...
        if (j_bound == 1) then !ï¬çΩêÖàÊ
            i1 = 2; i2 = nx - 1
        else if (j_bound == 2) then !äJï˙êÖàÊ
            i1 = 1; i2 = nx
        else if (j_bound == 3) then !è„ó¨äJï˙,â∫ó¨ï«
            i1 = 1; i2 = nx - 1
        else ! è„ó¨ï«,â∫ó¨äJï˙
...
!
! ----- è„â∫ó¨â∫ó¨êÖà ã´äEèåè -----
!
        if (j_bound == 2 .and. j_periodic == 1) then !é¸ä˙ã´äEèåè
            hs(1) = hs(nx - 1)
            hn(1) = eta(1) + hs(1)
            hs(nx) = hs(2)
            hn(nx) = eta(nx) + hs(nx)
        ...
        if ((j_bound == 2 .and. j_periodic == 0) .or. j_bound == 4) then !â∫ó¨í[äJï˙
            if (j_hdw == 3) then
                hn(nx) = hn(nx - 1)
            else
                hn(nx) = h_down
...
!
! 2éüå≥äiéqÉfÅ[É^Å[ÇÃÉTÉCÉYÇì«Ç›çûÇﬁ ni,nj
!
...
! 2éüå≥äiéqÉfÅ[É^Å[Çì«Ç›çûÇﬁ xg(ni,nj),yg(ni,nj)
...
! îwåiâ∑ìxÅEîwåiîZìxtmp0,con0ÇÃåvéZ
...
! â∫ó¨í[êÖà èåè 1==àÍíË, 2==Sineîgå`, 3==é©óR(dH/dx=0)
        if (j_hdw == 1) then
            call cg_iric_read_integer(fid, 'j_set_hdw', j_set_hdw, ier)
! àÍíËílÇÃèåè 0==èâä˙èåèÇ∆ìØÇ∂, 1==ílÇÃì¸óÕ
            if (j_set_hdw == 1) then
...
! Sineîgå`ÇÃ
            call cg_iric_read_real(fid, 'hd_amp', hd_amp, ier) !îgçÇ
            call cg_iric_read_real(fid, 'hd_wl', hd_wl, ier) !îgí∑
            call cg_iric_read_real(fid, 'hd_st', hd_st, ier) !äJénéûçè
            call cg_iric_read_real(fid, 'hd_ap', hd_ap, ier) !äJénéûçè
        end if
    end if
    if (j_bound == 1 .or. j_bound == 4) j_hup = 3
    if (j_qgive == 1) then !è„ó¨Ç©ÇÁó¨ó Çó^Ç¶ÇÈèÍçá
        j_hup = 3
        j_set_hup = 0
...
!è„ó¨êÖà ÇÃèåèÅ@1=àÍíËÅC 2=Sineîg, 3=é©óR(dH/dx=0)
        if (j_hup == 1) then
...
! Sineîgå`ÇÃ
            call cg_iric_read_real(fid, 'hu_amp', hu_amp, ier) !îgçÇ
            call cg_iric_read_real(fid, 'hu_wl', hu_wl, ier) !îgí∑
            call cg_iric_read_real(fid, 'hu_st', hu_st, ier) !äJénéûçè
            call cg_iric_read_real(fid, 'hu_ap', hu_ap, ier) !äJénéûçè
        end if
    end if
...
! é©óRêÖñ åvéZ åJÇËï‘ÇµâÒêî
    if (j_surf == 0) hloop = 1

    call cg_iric_read_real(fid, 'hloop_err', hloop_err, ier)
! é©óRêÖñ åvéZ åJÇËï‘ÇµåvéZë≈ÇøêÿÇËåÎç∑
...
!      write(44,*) 'i,j_inlen(i)=',i,j_inlen(i)    !ã´äEóvëfÇÃêî
...
!      h(1)=h(2)+(h(2)-h(3))                  !è„ó¨í[êÖà ÇÃèâä˙íl
!      h(0)=h(1)+(h(1)-h(2))                  !è„ó¨í[êÖà ÇÃèâä˙íl
        eta(1) = eta(2)
        eta(0) = eta(1)
        h(1) = h(2)
        h(0) = h(1)
...
!      h(nx)=h(nx-1)-(h(nx-2)-h(nx-1))        !â∫ó¨í[êÖà ÇÃèâä˙íl
!      h(nx+1)=h(nx)-(h(nx-1)-h(nx))        !â∫ó¨í[êÖà ÇÃèâä˙íl
        eta(nx) = eta(nx - 1)
        eta(nx + 1) = eta(nx)
        h(nx) = h(nx - 1)
        h(nx + 1) = h(nx)
...
!     end do
    ...
    hs_ave = hs_ave/float(nx)  ! hs_ave==äiéqÉfÅ[É^Ç©ÇÁåvéZÇµÇΩïΩãœêÖê[ÇÃèâä˙íl
    ! h0==äiéqÉfÅ[É^Ç©ÇÁì«ÇÒÇ≈Ç´ÇΩïΩãœêÖê[
    u_ave = 0.
...
!     yvo=yv
    ...
    do m = 1, hloop0
!      call v1cal(yu,yv)    !<---dune ÇÃåvéZÇÕÇ±Ç¡Çø, ñßìxó¨Ç‡Ç±Ç¡ÇøÇ›ÇΩÇ¢
        call v1cal(yun, yvn)  !<---Ç±ÇÍÇ…Ç∑ÇÈÇ∆é©óRêÖñ ÉZÉCÉVÉÖOK
!-------------------------------------------------
        if (j_surf == 1 .and. time > stime_surf) then
            call hcal                          ! h-->hn,hs
